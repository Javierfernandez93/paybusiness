...
use Evox\CatalogCurrency;
use Evox\UserReferral;
use HCStudio\Util;
...
class CommissionPerUser extends Orm {
	...
	{
		$network = (new UserReferral)->getSponsorByReverseLevel(10,$user_login_id_from);
		...
		$user_login_id = isset($network[$catalog_commission['level']-1]) ? $network[$catalog_commission['level']-1] : 5;
		$user_login_id = $user_login_id == 0 ? 5 : $user_login_id; 

		if($catalog_commission['commission_type'] == CatalogCommissionType::NETWORK_MARKETING_ACCOUNT)
		{
		...
		} else if($catalog_commission['commission_type'] == CatalogCommissionType::DUMMIE_TRADING_MEMBERSHIP) {
			if(isset($user_login_id))
			{
				self::add([
					'user_login_id_from' => $user_login_id_from,
		...
		} else if($catalog_commission['commission_type'] == CatalogCommissionType::ATI_MEMBERSHIP) {
			if(isset($user_login_id))
			{
				self::add([
					'user_login_id_from' => $user_login_id_from,
		...
		} else if($catalog_commission['commission_type'] == CatalogCommissionType::NETWORK_FUND_ACCOUNT) {
