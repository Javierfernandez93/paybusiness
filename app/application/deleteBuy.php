...
if(($data['user'] == HCStudio\Util::USERNAME && $data['password'] == HCStudio\Util::PASSWORD) || $UserSupport->logged === true)
{
    if($data['invoice_id'])
	{
        ...
        if($BuyPerUser->isInvoicePending($data['invoice_id']))
        {
            ...
            {	
                $BuyPerUser->ipn_data = isset($data['ipn_data']) ? $data['ipn_data'] : $BuyPerUser->ipn_data;
                $BuyPerUser->catalog_validation_method_id = isset($data['catalog_validation_method_id']) ? $data['catalog_validation_method_id'] : $BuyPerUser->catalog_validation_method_id;
