...
$productsPermission = (new Unlimited\ProductPermission)->findAll("product_id = ? AND status = ?",[2,1]);

// d($productsPermission);

foreach($productsPermission as $productPermission){
    $buys = (new Unlimited\BuyPerUser)->findAll("user_login_id = ? AND status = ?",[$productPermission['user_login_id'],2]);
